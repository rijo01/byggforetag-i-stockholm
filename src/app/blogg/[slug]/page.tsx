...
import { getAllBlogg, getBloggBySlug, getAllTjanster } from "@/lib/content";
...
export async function generateStaticParams() {
  return getAllBlogg().map((b) => ({ slug: b.meta.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const blogg = getBloggBySlug(slug);
  if (!blogg) return {};

  return {
    title: blogg.meta.metaTitle,
    description: blogg.meta.metaDescription,
    alternates: { canonical: `https://byggforetag-i-stockholm.se/blogg/${slug}` },
    openGraph: {
      title: blogg.meta.metaTitle,
      description: blogg.meta.metaDescription,
      url: `https://byggforetag-i-stockholm.se/blogg/${slug}`,
      locale: "sv_SE",
      type: "article",
    },
  };
...
export default async function BloggPage({ params }: Props) {
  ...
  const { meta, content } = blogg;
  const allBlogg = getAllBlogg().filter((b) => b.meta.slug !== slug);
  const relatedBlogg = allBlogg.slice(0, 3);
      ...
      <main className="flex-1">
        <div className="bg-gray-100 border-b">
          <div className="container-site py-3">
            <nav className="text-sm text-gray-500">
              ...
              <Link href="/blogg" className="hover:text-brand-600">Blogg</Link>
              <span className="mx-2">›</span>
        ...
        <section className="bg-gradient-to-br from-brand-800 to-brand-900 text-white py-16">
          <div className="container-site">
            ...
            {meta.publishedAt && (
              <p className="mt-4 text-brand-300 text-sm">Publicerad: {meta.publishedAt} · Uppdaterad: {meta.updatedAt || meta.publishedAt}</p>
            )}
          </div>
        </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
              ...
              <aside className="lg:col-span-1">
                <div className="sticky top-8 space-y-6">
                  {relatedBlogg.length > 0 && (
                    <div className="bg-white border border-gray-200 rounded-xl p-6">
                      <h3 className="font-display font-bold text-brand-900 mb-4">Senaste från bloggen</h3>
                      <ul className="space-y-3">
                        {relatedBlogg.map((b) => (
                          <li key={b.meta.slug}>
                            <Link href={`/blogg/${b.meta.slug}`} className="text-brand-600 hover:text-brand-800 flex items-center gap-2">
                              <span>→</span><span className="text-sm">{b.meta.title}</span>
                            </Link>
                          </li>
                        ))}
                      </ul>
